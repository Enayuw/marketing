...
if [[ $PINPOINT_ENABLE == 'true' ]] ; then
    ...
    if [[ "${PINPOINT_OPTIONS_VER}" == "new" ]] ; then
        PINPOINT_OPTIONS=" -javaagent:/opt/springcloud/data/pinpoint-1.8.4/pinpoint-bootstrap-1.8.4.jar -Dpinpoint.agentId=$AGENT_ID -Dpinpoint.applicationName=marketing-data-bridge-$SPEED_ENV "
    else
        PINPOINT_OPTIONS=" -javaagent:/opt/springcloud/data/pinpoint/pinpoint-bootstrap-1.8.4.jar -Dpinpoint.agentId=$AGENT_ID -Dpinpoint.applicationName=marketing-data-bridge-$SPEED_ENV "
    fi
else
    echo "未开启Pinpoint"
fi
