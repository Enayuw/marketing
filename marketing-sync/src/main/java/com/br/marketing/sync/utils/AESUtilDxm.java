...
public class AESUtilDxm {
    ...
    public static void decryptCSV(String inputPath, String outputPath, String keyHex) throws Exception {
        ...
        for (int i = 0; i < headers.length; i++) {
            String header = headers[i].trim().replaceAll("^\"|\"$", ""); // 去除可能的引号
                ...
                if (columns.length > phoneColumnIndex) {
                    ...
                    if (phoneColumn != null && !phoneColumn.trim().isEmpty()) {
                        try {
                            ...
                            System.err.println("【度小满解密】解密失败 (行 " + (i + 1) + ")，保留原值: " + e.getMessage());
                        }
                    } else {
                        System.out.println("【度小满解密】跳过空手机号 (行 " + (i + 1) + ")");
                    }
                }
    ...
    private static List<String> readCsvWithBomSupport(String inputPath) throws IOException {
        ...
        if (fileBytes.length >= 3 && 
            ...
            hasUtf8Bom = true;
            System.out.println("【度小满解密】检测到 UTF-8 BOM 标记，将自动处理");
        }
        ...
        for (Charset charset : charsets) {
            try {
                ...
                System.out.println("【度小满解密】使用 " + detectedEncoding + " 编码读取文件");
                break; // 成功读取，跳出循环
            } catch (Exception e) {
                // 继续尝试下一个编码
                continue;
