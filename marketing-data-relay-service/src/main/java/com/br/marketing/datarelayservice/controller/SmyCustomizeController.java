...
public class SmyCustomizeController {
    ...
    public SmyResponseDTO receiveSmyUploadData(@RequestBody SmyUploadRequestDTO dto) {
        return smyUploadDataService.receiveSmyUploadData(dto);
    }

    @ApiOperation(value = "萨摩耶代运营数据上传")
    @PostMapping("/transfer")
    @PrometheusTimeMethod(buckets = {0.05d, 0.1d, 0.2d, 0.5d}, methodType = MethodType.ACCESS, to = 0)
    public SmyResponseDTO receiveSmyTransferData(@RequestBody SmyTransferRequestDTO dto) {
        return smyTransferDataService.receiveSmyTransferData(dto);
    }
}
