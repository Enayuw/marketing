...
public class TransferFileTaskController {
    ...
    @ApiOperation(value = "重新执行转化提取", notes = "重新执行转化提取", httpMethod = "POST")
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "转化数据id", paramType = "query", dataType = "integer")})
    @ApiResponses(value = {@ApiResponse(code = 500, message = "INTERNAL_SERVER_ERROR", response = MarketingSyncReport.class)})
    public ApiResult<PageResultReturn> reStartTransfer(@RequestParam(required = true) int id) {
