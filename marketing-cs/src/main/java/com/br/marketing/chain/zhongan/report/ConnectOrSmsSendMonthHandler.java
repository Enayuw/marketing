...
public class ConnectOrSmsSendMonthHandler implements ZhongAnReportHandler {
    ...
    public boolean check(String cellMd5, String bizDate) throws Exception {
        List<Long> connectIds =
                Optional.ofNullable(zhongAnCollidingDataLogMapper.getConnectIdsByMonth(cellMd5, bizDate)).orElse(Collections.emptyList());
        List<Long> smsSendIds = Optional.ofNullable(zhongAnCollidingDataLogMapper.getSmsIdsByMonth(cellMd5, bizDate)).orElse(Collections.emptyList());
