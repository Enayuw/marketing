...
public class QiFuQrySleepUserRealMessageServiceImpl implements QiFuQrySleepUserRealMessageService{
    ...
    public void process(String apiCode) {
        ...
        for (String tskId : taskIdSet) {
            ...
            while (true) {
                // 循环获取条件数据，每次2000条
                // 根据手机号 筛选 未推送过的数据
                final List<MarketingSyncUser> pageList = marketingSyncUserMapper.getSyncUserByCusBatch(apiCode, tskId, indexId);
                if (CollectionUtils.isEmpty(pageList)) {
                    break;
                }
                indexId = pageList.get(pageList.size() - 1).getId();

                List<MarketingSyncUser> usersToSave = new ArrayList<>();
                for (MarketingSyncUser user : pageList) {
                    // 将用户添加到待保存列表中
                    usersToSave.add(user);

                    // 满50条推送
                    if (usersToSave.size() == 50) {
                        threadPool.execute(() -> {
                            action(pageList,apiCode,tskId);
                        });
                        // 清空待保存列表
                        usersToSave.clear();
                    }
    ...
    private void action(List<MarketingSyncUser> pageList,String apiCode,String tskId){
