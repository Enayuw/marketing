...
@Service
@Slf4j
public class DataWhiteListMarkServiceImpl implements DataWhiteListMarkService {
    ...
    public void process() {
        ...
        marketingCommonConfig.getDataMarkApiCodes().forEach((String apiCode) -> {
            ...
            String key = RedisKeyConstant.DATA_WHITELIST_MARK.concat(":").concat(apiCode);
            while (true) {
                String lockValue = UUID.randomUUID().toString();
                try {
                    redisChgService.lock(key, lockValue);
                    //打标表数据查询
                    List<FlagData> list = flagDataMapper.queryFlagWhiteListComputation(pageSize, apiCode);
                    ...
                    flagDataMapper.batchUpdateFlagWhiteListComputationByIds(ids, 0);
                    //释放锁
                    try {
                        redisChgService.unlock(key, lockValue);
                    } catch (Exception e) {
                        log.warn(AlertLog.buildWarnMessage(AlarmSendCodeEnum.PP_MARKING_SERVICEERROR.getCode(),
                                "pp停车白名单打标释放锁出现异常，" + "errorMessage=" + e.getMessage()), e);
                        flagDataMapper.batchUpdateFlagWhiteListComputationByIds(ids, null);
                        redisChgService.unlock(key, lockValue);
                        threadPoolShutDown(threadPool);
                        break;
                    }
                    //打标更新:FlagWhiteList
                    updateFlagWhiteList(threadPool, list);
                    threadPoolShutDown(threadPool);
                } catch (Exception e) {
                    log.warn(AlertLog.buildWarnMessage(AlarmSendCodeEnum.PP_MARKING_SERVICEERROR.getCode(),
    ...
    void updateFlagWhiteList(ThreadPoolExecutor threadPool, List<FlagData> ids) {
        List<List<FlagData>> partitions = Lists.partition(ids, PARTATION_SIZE);
        for (List<FlagData> partition : partitions) {
            threadPool.submit(() -> markAndUpdateFlagWhiteList(partition));
        }
    }

    void markAndUpdateFlagWhiteList(List<FlagData> list) {
        try {
