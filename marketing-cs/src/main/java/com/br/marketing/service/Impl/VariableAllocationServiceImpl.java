...
public class VariableAllocationServiceImpl implements VariableAllocationService {
    ...
    public PageResultReturn getVariableList(VariableAllocationDTO dto) {
        ...
        try {
            ...
            List<VariableAllocationVO> variableList = variableAllocationMapper.getVariableList(apiCode, allocationType);
            for (VariableAllocationVO variabl : variableList) {
                BigDecimal releaseTimeNum = variableAllocationMapper.getVariableAllocationVO(date);
                BigDecimal sum = variabl.getNormalQuantity();
                BigDecimal falseNum = sum.subtract(releaseTimeNum);
                variabl.setReleaseTimeNum(releaseTimeNum);
                variabl.setFalseNum(falseNum);
            }

            return PageResultReturn.setPageResult(variableList, dto.getCurrent(), dto.getSize());
        } catch (Exception e) {
    ...
    public ApiResult<Boolean> updateVariableList(Long id, BigDecimal normalQuantity, BigDecimal abnormalQuantity) {
        ...
        variableAllocationMapper.updateByPrimaryMutchKeySelective(allocationVO);
