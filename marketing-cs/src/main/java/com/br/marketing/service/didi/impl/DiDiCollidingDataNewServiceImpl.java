...
public class DiDiCollidingDataNewServiceImpl implements DiDiCollidingDataNewService {
    ...
    private final static String TITLE = "【滴滴V5-new-短信流量数据】";

    private ConcurrentHashMap<String, String> scasMap = new ConcurrentHashMap<>();

    private final AtomicReference<String> preScas = new AtomicReference<>();
    ...
    public void colliding(JobExecutionMultipleShardingContext context) {
        ...
        while (true) {
            ...
            boolean collidingSwitch = collidingConfig2.getBoolean("collidingSwitch") != null ? collidingConfig.getBoolean("collidingSwitch") : false;
            if (collidingSwitch) {
                break;
            }
            String startTimeStr = LocalDate.now().format(DateTimeFormatter.ofPattern(DatePattern.NORM_DATE_PATTERN)) + " " + collidingConfig.getString("firstBatchStartTime");
            String endTimeStr = LocalDate.now().format(DateTimeFormatter.ofPattern(DatePattern.NORM_DATE_PATTERN)) +  " " +collidingConfig.getString("firstBatchEndTime");
            ...
            int limit = collidingConfig2.getInteger("limit") != null ? collidingConfig.getInteger("limit") : 2000;
    ...
    private void collidingData(DiDiV5CollidingData data, String mediaName, String token, RateLimiter rateLimiter, List<String> retryHttpCode) {
        // 单个撞库异常不影响其他撞库
        try {
            // 尝试获取令牌
            ...
            boolean retry = retryHttpCode.contains(Integer.parseInt(httpcode));
            if(retry) {
                return;
            }
