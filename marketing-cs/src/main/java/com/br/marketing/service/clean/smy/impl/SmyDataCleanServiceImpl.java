...
public class SmyDataCleanServiceImpl implements SmyDataCleanService {
    ...
    public void cleanCustomizedTransferData(String apiCode, String date) {
        ...
        minId = minId - 1;
        while (mark) {
            List<CustomizeTransferDataSmy> transferDataSmyList = customizeTransferDataSmyMapper.smyCleanCustomizedUploadDataByMinId(tCid, apiCode,
                    date, minId, limit);
            ...
            threadPool.submit(() -> {
                for (CustomizeTransferDataSmy customizeTransferDataSmy : transferDataSmyList) {
                    SmyTransferRequestDTO smyUploadRequestDTO = JSONObject.parseObject(customizeTransferDataSmy.getRequestJsonData(),
                            SmyTransferRequestDTO.class);
                    Result<Boolean> result;
                    try {
                        ...
                        List<TransferDataItemDTO> dataTransferItems = buildTransferDataItems(smyUploadRequestDTO);
    ...
    private List<TransferDataItemDTO> buildTransferDataItems(SmyTransferRequestDTO smyUploadRequestDTO) {
