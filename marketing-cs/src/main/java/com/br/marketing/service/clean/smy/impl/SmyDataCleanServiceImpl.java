...
public class SmyDataCleanServiceImpl implements SmyDataCleanService {
    ...
    public void cleanCustomizedUploadData(String apiCode, String date) {
        ...
        Long minId = customizeUploadDataSmyMapper.smyCleanCustomizedUploadDataOfMinId(apiCode, date);
        if (minId == null) {
            return;
        }
        minId = minId - 1;
        while (mark) {
            List<CustomizeUploadDataSmy> uploadDataSmyList = customizeUploadDataSmyMapper.smyCleanCustomizedUploadDataByMinId(apiCode, date, minId,
                    limit);
                ...
                for (CustomizeUploadDataSmy customizeUploadDataSmy : uploadDataSmyList) {
                    ...
                    try {
                        ...
                        if (ResultCode.SUCCESS.getValue().equals(result.getCode())) {
                            customizeUploadDataSmy.setSyncStatus(1);
                            customizeUploadDataSmyMapper.updateByPrimaryKeySelective(customizeUploadDataSmy);
                        }
                    } catch (Exception e) {
